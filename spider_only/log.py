...
from .vars import CACHE_PATH, INFO, WARN, NAVI_NAME
...
@bp.route('/log/<opt>/<project>/<spider>/<job>/<ext>/', methods=('GET', 'POST'))
@bp.route('/log/<opt>/<project>/<spider>/<job>/', methods=('GET', 'POST'))
def log(opt, project, spider, job, ext=None):
    
    navis = NAVI_NAME
    TEMPLATE_UTF8 = 'log/utf8.html'
    TEMPLATE_STATS = 'log/stats.html'

    SCRAPYD_SERVER = app.config.get('SCRAPYD_SERVER', '127.0.0.1:6800')
    SCRAPYD_SERVERS_AUTH = app.config.get('SCRAPYD_SERVERS_AUTHS', None)
    ...
    ENABLE_CACHE = True if not DISABLE_CACHE else False

    if ENABLE_CACHE:
        node_path = os.path.join(CACHE_PATH, re.sub(r'\.|\:', '_', SCRAPYD_SERVER))
    ...
    if not text:
        ...
        for ext in SCRAPYD_LOG_EXTENSIONS:
            ...
            if status_code == 200:
                break
        # if status_code != 200:
        #     return render_template(TEMPLATE_RESULT,
    ...
    if ENABLE_CACHE:
        ...
        refresh_url = request.script_root + request.path + '?no_cache=True'
    else:
        refresh_url = ''
    ...
    html_utf8 = render_template(TEMPLATE_UTF8,navis=navis,
    ...
    html_stats = render_template(TEMPLATE_STATS, **kwargs)
